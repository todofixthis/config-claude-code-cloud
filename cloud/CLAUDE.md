<!-- GENERATED FILE — do not edit directly. Edit cloud/CLAUDE.cloud-only.md in todofixthis/config-claude-code-cloud, or CLAUDE.shared.md in todofixthis/config-claude, then re-run cloud/generate-claude-md.sh. -->
...
For an issue — its body or a comment on it — add the footer yourself; nothing else attributes it to you. Anything on a PR — its description, a comment, or a review — differs: in a cloud/web session, Anthropic's GitHub proxy appends its own "Generated by Claude Code" footer after the fact, a step you never see happen, so your own footer would only stack beneath it. Skip your own footer on a PR in a cloud/web session; the proxy's stands in for it. A local CLI session has no such proxy, so add the footer to PR items too, exactly as for issues.
