...
Either way, in a `todofixthis/config-claude-code-cloud` checkout: run `cloud/generate-claude-md.sh`, commit `cloud/CLAUDE.md`, and push. Then bump the version comment in `cloud/pointer.sh` and re-paste `pointer.sh`'s content into the environment's Setup script field, to apply the change immediately rather than waiting for the ~7-day cache cycle. That repo's `cloud/setup.sh` is unaffected — it still fetches `cloud/CLAUDE.md` as one file; this split only changes how that file is built.
