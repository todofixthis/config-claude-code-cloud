...
For issues, reviews, and comments, add the footer yourself — nothing else attributes those to you. A PR description differs: in a cloud/web session, Anthropic's GitHub proxy appends its own "Generated by Claude Code" footer once the PR is created — a step you never see happen — so your own footer would only stack beneath it, not replace it. Skip your own footer on a PR description there; the proxy's stands in for it. A local CLI session has no such proxy, so add the footer to PR descriptions too, exactly as everywhere else.
