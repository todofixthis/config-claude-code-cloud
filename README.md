...
- `.github/workflows/generate-claude-md.yml` — runs the above automatically on a push to `main` that touches `cloud/CLAUDE.cloud-only.md`, using the `CONFIG_CLAUDE_READ_TOKEN` secret below to authenticate the fetch, and commits the regenerated `cloud/CLAUDE.md`.
...
`.github/workflows/generate-claude-md.yml` needs a token with read access to `todofixthis/config-claude` (private), to fetch `CLAUDE.shared.md` — the default `GITHUB_TOKEN` a workflow run here gets is scoped to this repo only, and this repo never keeps its own copy of that file to fall back on. This is a GitHub Actions repository secret, not something the cloud session sandbox itself holds — it's unrelated to, and doesn't relax, the "no secrets store" rule under "What's deliberately not here" below, which is about that sandbox specifically. Set it up once:
