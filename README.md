...
- `cloud/CLAUDE.shared.md` — the personal instructions shared with `todofixthis/config-claude` (the local-machine counterpart), canonical there. Synced automatically into this repo by that repo's `.github/workflows/publish-shared-claude-md.yml` — don't edit here.
...
- **Editing `CLAUDE.cloud-only.md`**: run `cloud/generate-claude-md.sh` to regenerate `cloud/CLAUDE.md`, then follow the bump-and-re-paste step above.
- **Editing shared content**: not here — edit `todofixthis/config-claude`'s `CLAUDE.shared.md` instead; a GitHub Actions workflow there publishes the change into this repo automatically (see `cloud/CLAUDE.cloud-only.md` for the full procedure).
